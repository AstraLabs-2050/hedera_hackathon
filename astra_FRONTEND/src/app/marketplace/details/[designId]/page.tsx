...
import { formatCurrency } from "./../../../dashboard/(with-sidebar)/utils/format";
import { redirect } from "next/navigation";
import ChatWithCreatorButton from "../../../components/ChatWithCreator";
...
export default function DesignPage({ params }: { params: { designId: string } }) {
  const designId = params.designId;
  const [design, setDesign] = useState<Design | null>(null);

  useEffect(() => {
    if (!designId) redirect("/marketplace");
      ...
      <div className="max-w-5xl py-16 mx-auto p-6 rounded-3xl overflow-hidden shadow-xs">
        <div className="flex flex-col gap-5 md:flex-row">
          ...
          <div className="md:w-1/2 p-6 space-y-4 border border-primary/40 rounded-2xl">
            ...
            <div className="space-y-2">
              <p className="text-sm font-semibold text-primary/90">Ships from</p>
              <div className="inline-block bg-gray-100 text-gray-800 px-4 py-2 rounded-full text-sm border border-gray-300">
                {design.shipsFrom}
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex space-x-2">
                ...
                {/* Directly use chat button (handles auth inside) */}
                <ChatWithCreatorButton
                  creatorId={design.creatorId || "e840f26f-6490-4dc6-940d-3ec95b1e381a"}
                  designId={String(design.id)}
                  jobData={{
                    title: design.title,
                    // imageUrl:design.imageUrl,
                    // description: design.description,
                    // timeline: design.deliveryWindow,
                    price: design.price,
                  }}
                />
              </div>
