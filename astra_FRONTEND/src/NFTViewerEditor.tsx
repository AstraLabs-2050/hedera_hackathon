...
const NFTViewerEditor: React.FC<NFTViewerEditorProps> = ({ contractAddress, client }) => {
  ...
  const fetchNFTData = async () => {
    ...
    try {
      ...
      try {
        ...
        owner = await contract.read.ownerOf([tokenIdNumber]);
      
        
        // Get the NFT metadata using the getter functions
        const designId = await contract.read.getDesignId([tokenIdNumber]);
        const designName = await contract.read.getDesignName([tokenIdNumber]);
        const fabricType = await contract.read.getFabricType([tokenIdNumber]);
        const designImage = await contract.read.getDesignImage([tokenIdNumber]);
        const prompt = await contract.read.getPrompt([tokenIdNumber]);
        
        // Get previous owners and users
        const previousOwners = await contract.read.getPreviousOwners([tokenIdNumber]);
        const usersOfDesign = await contract.read.getUsersOfDesign([tokenIdNumber]);
  ...
  const updateMetadata = async (field: 'designName' | 'fabricType' | 'designImage' | 'prompt') => {
    if (!contract || !nftData || !address) return;
    ...
    try {
      ...
      // Call the update function using thirdweb
      const transaction = prepareContractCall({
        contract,
        method: functionName,
        params: [nftData.id, newValue]
      });
  ...
  const transferNFT = async () => {
    if (!contract || !nftData || !address || !transferToAddress) return;
    ...
    try {
      ...
      // Call the transfer function using thirdweb
      const transaction = prepareContractCall({
        contract,
        method: "function transferNFT(address to, uint256 tokenId) returns (bool)",
        params: [transferToAddress, nftData.id]
      });
  ...
  const addUserToDesign = async () => {
    if (!contract || !nftData || !address || !userToAdd) return;
    ...
    try {
      ...
      // Call the add user function using thirdweb
      const transaction = prepareContractCall({
        contract,
        method: "function addUserOfDesign(uint256 tokenId, address user) returns (bool)",
        params: [nftData.id, userToAdd]
      });
                ...
                <div className="value">
                  {nftData.previousOwners.length > 0 ? (
                    <ul className="address-list">
                      {nftData.previousOwners.map((prevOwner:any, index:number) => (
              ...
              <div className="info-row">
                ...
                <div className="value">
                  {nftData.usersOfDesign.length > 0 ? (
                    <ul className="address-list">
                      {nftData.usersOfDesign.map((user:any, index:number) => (
