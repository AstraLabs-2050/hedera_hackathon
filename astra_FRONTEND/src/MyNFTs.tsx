...
const MyNFTs: React.FC<MyNFTsProps> = ({ contractAddress, client }) => {
  ...
  const checkAndAddToken = async (tokenId: number) => {
    ...
    try {
      ...
      // Use the contract instance to call read methods
      ...
      if (owner.toLowerCase() === address.toLowerCase()) {
        ...
        // Get the metadata using contract read methods
        const designName = await contract.read.getDesignName([tokenId]);
        const fabricType = await contract.read.getFabricType([tokenId]);
        const designImage = await contract.read.getDesignImage([tokenId]);
        const prompt = await contract.read.getPrompt([tokenId]);
