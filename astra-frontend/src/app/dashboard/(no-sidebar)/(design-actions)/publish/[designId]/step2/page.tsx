...
export default function PublishToMarketplaceBrandStoryPage({
  ...
  useEffect(() => {
    setMounted(true);
    if (!isEditMode) {
      const step2Data = getPublishData(designId, "step2");
      if (step2Data) form.reset(step2Data);
    }
  }, [form, isEditMode, designId]);

  // Auto-save while typing (only in non-edit mode)
  useEffect(() => {
    if (mounted && !isEditMode) {
      const subscription = form.watch((value) => {
        savePublishData(designId, "step2", value);
      });
      return () => subscription.unsubscribe();
    }
  }, [form, mounted, isEditMode, designId]);
  ...
  const onPreviewListing = async (data: BrandStoryFormData) => {
    ...
    try {
      // Save step 2 data
      savePublishData(designId, "step2", data);
  ...
  const onSaveAsDraft = async () => {
    ...
    try {
      ...
      // Save current data
      savePublishData(designId, "step2", formData);
  ...
  const handleSaveChanges = async () => {
    ...
    // For edit mode, we might want to update the design directly
