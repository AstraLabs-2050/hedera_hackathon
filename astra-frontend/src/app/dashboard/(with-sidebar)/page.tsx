...
import ListedDesignCard from "@/app/components/ListedDesignCard";
...
function Page() {
  ...
  const [listedDesigns, setListedDesigns] = useState<Design[]>([]);
  ...
  useEffect(() => {
    const fetchListedDesigns = async () => {
      try {
        setLoadingDesigns(true);
        const response = await api.getCreatorInventory();
        if (response.status && response.data) {
          // Filter only "listed" designs
          const listed = response.data.filter(
            (d) => d.publishedStatus === "listed"
          );
          setListedDesigns(listed);
        }
      } catch (err) {
        console.error("Failed to fetch listed designs:", err);
      } finally {
      ...
      <div className='bg-white p-6 md:py-10 md:px-10 lg:px-12 2xl:px-20 rounded-2xl border border-gray-200'>
        <div className='space-y-10'>
          <div className='flex justify-between items-center'>
            <h2 className='text-xl font-medium'>Listed Designs</h2>
            <Link
            ...
            <div className='flex justify-center items-center h-auto md:h-[400px]'>
              <div className='space-y-2 text-center'>
                <p className='text-black/50 font-[ClashGrotesk-regular]'>
                  You have no designs listed on the marketplace
                </p>
